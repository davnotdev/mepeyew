...
fn main() {
    ...
    let vs = include_bytes!("shaders/instancing/vs.wgsl");
    let fs = include_bytes!("shaders/instancing/fs.wgsl");
    ...
    let (ubo, ubo_guard) = context
        .new_uniform_buffer(&UniformData::default(), None)
        .unwrap();
    ...
    let program = context
        .new_program(
            ...
            &[ShaderUniform {
                set: 0,
                binding: 0,
                ty: ShaderUniformType::UniformBuffer(ubo),
            }],
            None,
        )
        .unwrap();
